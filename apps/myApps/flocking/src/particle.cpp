...
void particle::update(){	
    ...
    //vel = vel + frc;
    if (pos.x > ofGetWidth()/5*3 || pos.x < ofGetWidth()/5*2
        || pos.y > ofGetHeight()/5*3 || pos.y < ofGetHeight()/5*2
        || pos.z > 300 || pos.z < -300)
    {
        vel = vel + frc + (ofVec3f(ofGetWidth()/2,ofGetHeight()/2,0) - pos) /1000;
    }
    else {
        vel = (vel + frc)*0.9;
    }
...
void particle::draw(){
    ...
    velPerp.z = velNormal.z;
	
	//ofLine(pos.x, pos.y, pos.z, pos.x + velNormal.x*10, pos.y + velNormal.y*10, pos.z + velNormal.z*10);
}
